...
public class ProjectService {
    ...
    public PagedResponse<ProjectListDto> findAll(
            String buId, Long customerId, String status,
            Short year, String search, int page, int size) {

        Pageable pageable = PageRequest.of(page, size, Sort.by("createdAt").descending());
        String filteredBuId = (buId == null || buId.isBlank()) ? null : buId;
        Short filteredYear = (year == null || year == 0) ? null : year;
        ProjectStatus filteredStatus = (status == null || status.isBlank()) ? null : ProjectStatus.fromDbValue(status);

        Page<Project> result = filteredYear == null
    ...
    public ProjectDetailDto create(ProjectCreateRequest req) {
        FrontFinancier ff = frontFinancierRepo.findByCodeIgnoreCase(req.getFrontFinancier())
        ...
        ProjectFunction fn  = (req.getFunctionName() != null && !req.getFunctionName().isBlank())
            ? functionRepo.findByNameIgnoreCase(req.getFunctionName()).orElse(null) : null;

        Short year = req.getProjectYear() != null
            ? req.getProjectYear() : (short) Year.now().getValue();

        // ✅ Nouveau format : FF_Code - BU_ID - BU_Trigram - Industry_Trigram - Customer_Trigram - Activity
        String name = buildName(ff.getCode(), bu.getId(), bu.getTrigram(),
            ind.getTrigram(), cust.getTrigram(), req.getActivity());
        ...
        Project p = Project.builder()
            ...
            .technicalOffice(TechnicalOffice.fromDbValue(req.getTechnicalOffice()))
            .status(ProjectStatus.fromDbValue(req.getStatus()))
    ...
    public DashboardStatsDto getDashboardStats(Short year) {
        ...
        BigDecimal revenue = projects.stream()
            .map(p -> nvl(p.getRevenueBudget())).reduce(BigDecimal.ZERO, BigDecimal::add);
        BigDecimal cost = projects.stream()
            .map(p -> nvl(p.getCostBudget())).reduce(BigDecimal.ZERO, BigDecimal::add);
    ...
    // ✅ Nouveau format avec 6 paramètres + limite 50 caractères
    private static String buildName(String ffCode, String buId, String buTrigram,
                                     String indTrigram, String custTrigram, String activity) {
        String name = String.join(" - ",
            trim(ffCode), trim(buId), trim(buTrigram),
            trim(indTrigram), trim(custTrigram), trim(activity));
